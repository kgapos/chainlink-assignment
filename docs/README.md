...
This stack runs two VeChain testnet nodes behind Nginx and a full observability pipeline
...
- Public VeChain API (Nginx): http://localhost:80
...
1. Prometheus targets: http://localhost:9090/targets
   - Expect `vechain-node` targets (`node-a:2112`, `node-b:2112`) and `prometheus` to be up.
2. Grafana dashboard:
...
- API is public via Nginx only; node P2P ports stay internal to Docker network.
